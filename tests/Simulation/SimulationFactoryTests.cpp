...
TEST_CASE("Simulation Config Invalid inputs") {
    ...
    CHECK_THROWS_AS(gol::config::SimulationConfig(-1, gol::config::GridMode::Dense, gol::config::TickMode::SingleCore,
                                                          gol::config::EdgeMode::Toroidal,
                                                          gol::config::Ruleset::Conway), std::invalid_argument);
    CHECK_THROWS_AS(gol::config::SimulationConfig(10001, gol::config::GridMode::Dense, gol::config::TickMode::SingleCore,
                                                          gol::config::EdgeMode::Toroidal,
                                                          gol::config::Ruleset::Conway), std::invalid_argument);
    auto validConfig = gol::config::SimulationConfig(3, gol::config::GridMode::Dense, gol::config::TickMode::SingleCore,
                                                          gol::config::EdgeMode::Toroidal,
...
TEST_CASE("Simulation Factory Enums coverage") {
    for (const auto grid : magic_enum::enum_values<gol::config::GridMode>()) {
        for (const auto tick : magic_enum::enum_values<gol::config::TickMode>()) {
            for (const auto edge : magic_enum::enum_values<gol::config::EdgeMode>()) {
                for (const auto rules : magic_enum::enum_values<gol::config::Ruleset>()) {
                    gol::config::SimulationConfig config(3, grid, tick, edge, rules);
